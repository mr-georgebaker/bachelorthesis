...
// and due to the external potential
  ...
  //////////////////////////  
  for (int i=0; i<3; ++i){
    forces[i] += spring*((positions[i+3]+boxes[i+3]*region_length) - (positions[i]+boxes[i]*region_length));
    forces[3*amount-3+i] += spring*((positions[3*amount-6+i]+boxes[3*amount-6+i]*region_length) - (positions[3*amount-3+i]+boxes[3*amount-3+i]*region_length));
  }
  #ifdef _OPENMP
    omp_set_num_threads(threads);
    #pragma omp parallel for
  ...
  for (int i=1; i<(amount-1); ++i){
    for (int j=0; j<3; ++j){
      forces[3*i+j] += spring*((positions[3*i+3+j]+boxes[3*i+3+j]*region_length) - 2*(positions[3*i+j]+boxes[3*i+j]*region_length) + (positions[3*i-3+j]+boxes[3*i-3+j]*region_length));
    }
  }
  #pragma omp barrier
  //////////////////////////////////////
