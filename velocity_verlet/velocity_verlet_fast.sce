...
// The difference between velocity_verlet.sce and velocity_verlet_fast.sce 
// is that velocity_verlet_fast.sce returns just the position
...
//          v: nxd matrix (velocities where d is the dimension)
    ...
    x(1,1:d) = x0
    for i = 1:1:n-1
        v_half = v0 + (1/2)*dt*(1/m)*forcefield(x(i,1:d)) - (1/2)*dt*g*v0 + (1/2)*sqrt(dt)*sigma*grand(1,d,"nor",0,1) - (1/8)*dt^2*g*((1/m)*forcefield(x(i,1:d)) - g*v0) - (1/4)*dt^(3/2)*g*sigma*((1/2)*grand(1,d,"nor",0,1) + (1/sqrt(3))*grand(1,d,"nor",0,1))
        x(i+1,1:d) = x(i,1:d) + dt*v_half + dt^(3/2)*sigma*(1/sqrt(12))*grand(1,d,"nor",0,1)
        v0 = v_half + (1/2)*dt*(1/m)*forcefield(x(i+1,1:d)) - (1/2)*dt*g*v_half + (1/2)*sqrt(dt)*sigma*grand(1,d,"nor",0,1) - (1/8)*dt^2*g*((1/m)*forcefield(x(i+1,1:d)) - g*v_half) - (1/4)*dt^(3/2)*g*sigma*((1/2)*grand(1,d,"nor",0,1) + (1/sqrt(3))*grand(1,d,"nor",0,1))
    end
endfunction
